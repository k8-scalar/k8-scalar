...
if [ $baseline == "true" ]; then
	...
	echo "Single increment stress testing"
	kubectl cp normal-single-increment-experiment.properties experiment-controller-0:/exp/etc
	mkdir -p output
	rm output/normal.sh
	touch output/normal.sh
	chmod +x output/normal.sh
	echo "java -jar /exp/lib/scalar-1.0.0.jar /exp/etc/platform.properties /exp/etc/normal-single-increment-experiment.properties" >> output/normal.sh
