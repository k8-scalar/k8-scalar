...
do
    ...
    now3=$(date +"%T")
    echo "${now3}: Let environment settle for 60seconds"
    sleep 60

    now4=$(date +"%T")
    echo "${now4}: Start the scalar run && surge-validator"
    bash exp-scalar-run.sh --incremental 60 ${duration} > experiment.out 2> experiment.err < /dev/null
    sleep 60
    ...
    kubectl cp experiment-controller-0:results--exp-etc-normal-incremental-experiment-properties.dat incremental-runs/run-${i}/run-13.dat
