...
echo -n "${BASH_SOURCE[0]} [OPTIONS] [ARGS]
 ...
 Parameters:
   userload				Specify the start user load, end user load and incrementing interval. (Format: NN:NN:NN)
...
do
	info "Stressing Cassandra with ${user_load} requests per second for ${flag_duration} seconds"
	run $user_load $flag_duration
done